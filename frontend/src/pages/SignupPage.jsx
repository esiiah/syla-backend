...
const countryCodes = [
  ...
  { code: "+61", country: "AU" }, { code: "+27", country: "ZA" },
  { code: "+234", country: "NG" }, { code: "+260", country: "ZM" }
];
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/auth/signup", {
        ...
        credentials: 'include',
        body: JSON.stringify({ 
          name, contact: finalContact, password,
          confirm_password: confirmPassword, contact_type: contactType
        })
      });
...
export default function SignupPage() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // Store auth data in both cookies (done by backend) and localStorage (for backward compatibility)
      localStorage.setItem("token", data.access_token);
      localStorage.setItem("user", JSON.stringify(data.user));
