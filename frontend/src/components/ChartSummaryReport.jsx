...
export default function ChartSummaryReport({ summary = {}, data = [], chartTitle = "" }) {
  ...
  return (
    <div className="mt-6 rounded-xl sm:rounded-2xl bg-white border border-gray-200 shadow-lg dark:bg-slate-900 dark:border-slate-700 overflow-hidden w-full max-w-full">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-3 sm:p-5 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        >
          <FileText size={18} className="sm:w-6 sm:h-6 flex-shrink-0" />
          <div className="min-w-0 flex-1">
            <h2 className="text-sm sm:text-xl font-bold flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
              Data Summary
              <span className="text-xs sm:text-sm font-normal text-blue-100">
                {isMinimized ? '(Tap to expand)' : '(Tap to collapse)'}
              </span>
            </h2>
            <p className="text-xs sm:text-sm text-blue-100">
              {isMinimized ? 'Statistical overview hidden' : 'Statistical overview'}
            </p>
          </div>
        </button>
        <button
          onClick={exportPDF}
          className="flex items-center gap-2 px-3 py-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-lg font-medium transition-all shadow-lg hover:shadow-xl text-xs sm:text-sm flex-shrink-0"
        >
          <Download size={16} />
